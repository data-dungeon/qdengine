...
#ifndef _TEMPLATE_OUT1_IN1_H_
#define _TEMPLATE_OUT1_IN1_H_

#ifdef PLATFORM_PS3 // todo
	#include <stdint.h>
	struct JOB_PARAM_OUT1_IN1
	{
...
#ifdef MAIN_LOOP
#define myCREATE_MAINJOB(JOB_NAME, OUT_TYPE, IN1_TYPE)		\
...
void	JOB_NAME(void *			pO,							\
	...
	#undef spu_printf
	#define spu_printf /*spu_printf*/

	uint32_t buf[128/sizeof(uint32_t)] __attribute__((aligned(128)));
	JOB_PARAM_OUT1_IN1 jobParams __attribute__ ((aligned(128)));

	#define CREATE_MAINJOB(JOB_NAME, OUT_TYPE, IN1_TYPE)																					\
		CREATE_OUT_STATIC_ARRAY(OUT_TYPE)																									\
		CREATE_IN_STATIC_ARRAY(1, IN1_TYPE)																									\
		myCREATE_MAINJOB(JOB_NAME, OUT_TYPE, IN1_TYPE)																						\
																																			\
		...
		{																																		\
			...
			{																																	\
				...
				{																																\
					...
					{																															\
						spu_printf("Start transfert of next input %X\n", (int)currentInput1);							\
						spu_printf("Destination: %X\n", (int)g_pIn1[nextUsedBuffer]);							\
					...
					JOB_NAME(g_pOut[currentUsedBuffer], g_pIn1[currentUsedBuffer], COUNT_BY_LOOP);					\
																																				\
			...
			{																																	\
				...
				cellDmaWaitTagStatusAll(DMA_DOWNLOAD_MASK(1));															\
																																				\
				JOB_NAME(g_pOut[currentUsedBuffer], g_pIn1[currentUsedBuffer], dataRemaining);						\
																																				\
