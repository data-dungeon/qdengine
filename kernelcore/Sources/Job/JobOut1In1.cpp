...
#include	"Root.h"
#include	INCL_KCORE(Job/JobOut1In1)
...
//-----------------------------------------------------------------------------
//	Name:		JOB_OUT1_IN1 constructor
//	Object:		
//	06-01-13:	SBE - Created
//-----------------------------------------------------------------------------
JOB_OUT1_IN1::JOB_OUT1_IN1(	PTR_JOB_FUNC	pJN
#ifdef PLATFORM_PS3
							,const char* pSpuElf
#endif
							)
...
//-----------------------------------------------------------------------------
//	Name:		~JOB_OUT1_IN1 destructor
//	Object:		
//	06-01-16:	SBE - Created
//-----------------------------------------------------------------------------
JOB_OUT1_IN1::~JOB_OUT1_IN1()
{
...
//	06-01-13:	SBE - Created
//-----------------------------------------------------------------------------
void	JOB_OUT1_IN1::SetOutPointer(void *	pPtr)
{
	_pOut = pPtr;
}
...
//	06-01-16:	SBE - Created
//-----------------------------------------------------------------------------
void	JOB_OUT1_IN1::SetOutPointerSize(unsigned int	nSize)
{
	_nSizeOfOut = nSize;
}
...
//	06-01-13:	SBE - Created
//-----------------------------------------------------------------------------
void	JOB_OUT1_IN1::SetInPointer(void *	pPtr)
{
	_pIn = pPtr;
}

//-----------------------------------------------------------------------------
//	Name:		SetInPointerSize
//	Object:		
//	06-01-16:	SBE - Created
//-----------------------------------------------------------------------------
void	JOB_OUT1_IN1::SetInPointerSize(unsigned int	nSize)
{
	_nSizeOfIn = nSize;
}
...
//	06-01-13:	SBE - Created
//-----------------------------------------------------------------------------
void	JOB_OUT1_IN1::SetCount(unsigned int	nC)
{
	_nCount = nC;
}
...
//	06-01-16:	SBE - Created
//-----------------------------------------------------------------------------
void	JOB_OUT1_IN1::Execute(unsigned int n)
{
#ifndef PLATFORM_PS3
	(*_pfJobName)(_pOut, _pIn, _nCount);
#else
	_nNbrSPUAvailable = n;
	...
	unsigned char *	pIn1 = (unsigned char *) _pIn;
	...
	for (i=0; i < _nNbrSPUAvailable; i++) 
	{
		...
		_jobParam_Out1In1[i].pOut = (uint64_t) pOut;
		_jobParam_Out1In1[i].pIn1 = (uint64_t) pIn1;
		_jobParam_Out1In1[i].nCount = (nCount / _nNbrSPUAvailable);

		pOut += _jobParam_Out1In1[i].nCount * _nSizeOfOut;
		pIn1 += _jobParam_Out1In1[i].nCount * _nSizeOfIn;

		_task_arg[i].u64[0] = (uint64_t)&_jobParam_Out1In1[i];
		_task_arg[i].u64[1] = i;
...
//-----------------------------------------------------------------------------
int		JOB_OUT1_IN1::Wait()
{
#ifdef PLATFORM_PS3
	if (_nCountRest > 0)
	{
		(*_pfJobName)(_pOutRest, _pInRest, _nCountRest);
	}
