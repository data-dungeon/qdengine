...
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::Open(const char * const	pFileName,
						  const F_MODE			eMode,
						  const bool			bSharing)
{
	...
	if ( TestStatus(F_STATUS::FS_OPEN) == false )
	{
		...
		_Handle = CreateFile(pFileName, dwDesiredAccess, dwSharingMode, NULL, dwCreationDisposition, dwFlagsAndAttributes, NULL);
...
//	02-02-18:	AMA - Created
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::IsExist(const char * const	pFileName) const
{
	WIN32_FIND_DATA FindFile;
	HANDLE handle = FindFirstFile(pFileName, &FindFile);
...
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::IsWriteProtected(const char * const	pFileName) const
{
	WIN32_FIND_DATA FindFile;
	HANDLE handle = FindFirstFile(pFileName, &FindFile);
	if (handle == INVALID_HANDLE_VALUE)
...
//	02-02-18:	AMA - Created
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::Seek(const F_SEEK	eInitialPosition,
...
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::Read(void *				pBuffer,
...
//	02-02-20:	AMA - Created
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::ReadInternal(void *		pBuffer,
						  const unsigned int		nPosition,
						  const int		nSize,
						  const F_SYNC	eSync)
		...
		do
		{
			...
			if ( bResult == FALSE )
			{
				...
				else
				{
					...
					QDT_Error("FILE_PC_W2K::ReadInternal() %s : %s", sMessage.GetBuffer(), sName.GetBuffer() );
...
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::Write(void *		pBuffer,
						  const unsigned int		nPosition,
						  const int		nSize,
						  const F_SYNC	eSync)
		...
		if ( bResult == FALSE )
		{
			...
			else
			{
				...
				else
				{
					...
					QDT_STRING sMessage;
					OS::GetWinLastErrorMsg(sMessage);
					QDT_Error("FILE_PC_W2K::Write() %s : %s", sMessage.GetBuffer(), sName.GetBuffer() );
				}
			}
		}
	}
...
//	02-02-20:	AMA - Created
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::Delete(const char * const	pFileName) const
{
	BOOL bError;
...
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::Rename(const char * const	pFileName) const
{
	BOOL bError;
...
//-----------------------------------------------------------------------------
bool	FILE_PC_W2K::IsIdle() const
{
	if ( FILES_MANAGER_ABC::Instance()->GetEnableEmulateCD() )
	{
		((FILE_PC_W2K*)this)->UpdateRead();
...
//	03-05-22:	VMA - Created
//-----------------------------------------------------------------------------
int		FILE_PC_W2K::GetSectorSize() const
{
	return SECTOR_SIZE;
}
...
//-----------------------------------------------------------------------------
void	FILE_PC_W2K::SetName(const QDT_STRING &	sName)
{
	_sName = sName;
}
...
//	03-09-10:	VPI - Created
//-----------------------------------------------------------------------------
const QDT_STRING &	FILE_PC_W2K::GetName() const
{
	return	(_sName);
}
...
//-----------------------------------------------------------------------------
unsigned int	FILE_PC_W2K::GetCurrentReadPosition() const
{
	return 0;
