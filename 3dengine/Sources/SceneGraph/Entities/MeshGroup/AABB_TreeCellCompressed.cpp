...
//	03-09-01:	RMA - Created
//-----------------------------------------------------------------------------
AABB_TREE_CELL_COMPRESSED::AABB_TREE_CELL_COMPRESSED()
:
_pLeftChild(NULL),
_pRightChild(NULL),
_nNbrIndices(0),
...
//-----------------------------------------------------------------------------
void	AABB_TREE_CELL_COMPRESSED::SetIndex(unsigned short	i,
											unsigned int	nIndex)
{
...
//	03-09-04:	RMA - Created
//-----------------------------------------------------------------------------
void	AABB_TREE_CELL_COMPRESSED::CreateChilds()
{
	QDT_ASSERT(_pLeftChild == NULL);
	QDT_ASSERT(_pRightChild == NULL);

	_pLeftChild = new AABB_TREE_CELL_COMPRESSED;
	_pRightChild = new AABB_TREE_CELL_COMPRESSED;
}
...
//	03-09-04:	RMA - Created
//-----------------------------------------------------------------------------
void	AABB_TREE_CELL_COMPRESSED::CreateIndices(unsigned int	nNbrIndices)
{
	QDT_ASSERT(_pIndices == NULL);
	QDT_ASSERT(nNbrIndices > 0);
	
	_nNbrIndices = nNbrIndices;
	_pIndices = new unsigned short [_nNbrIndices];
}
...
//-----------------------------------------------------------------------------
void	AABB_TREE_CELL_COMPRESSED::Display(VECTOR * pPoints)
{
	QDT_NOT_IMPLEMENTED();
...
//	_BoundingBox.Display(Green);
	...
	if (_pIndices)
	{
		...
		{
			unsigned short n1 = _pIndices[i * 3];
			unsigned short n2 = _pIndices[i * 3 + 1];
			unsigned short n3 = _pIndices[i * 3 + 2];
...
//	03-09-05:	RMA - Created
//-----------------------------------------------------------------------------
void	AABB_TREE_CELL_COMPRESSED::TestZone(VECTOR *		pPoints,
											VERTICAL_RAY &	Ray,
											bool &			bAbove,
											bool &			bBelow) const
{
...
//-----------------------------------------------------------------------------
bool	AABB_TREE_CELL_COMPRESSED::RayCast(VECTOR * pPoints, 
...
//	03-09-04:	RMA - Created
//-----------------------------------------------------------------------------
void	AABB_TREE_CELL_COMPRESSED::Read(IO_STREAM &	Stream)
{
	Stream >> _BoundingBox;
	...
	{
		CreateChilds();
		_pLeftChild->Read(Stream);
		_pRightChild->Read(Stream);
	}
}	
...
//-----------------------------------------------------------------------------
#ifndef _MASTER
void	AABB_TREE_CELL_COMPRESSED::Write(IO_STREAM &	Stream)
{
