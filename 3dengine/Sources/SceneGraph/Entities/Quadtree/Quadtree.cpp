...
#include	"Root.h"
#include	INCL_3DENGINE(SceneGraph/Entities/Quadtree/Quadtree)
...
QUADTREE::QUADTREE()
:
ENTITY_ABC	(GetGenericCommunicatorDescriptor(), ENTITY_QUADTREE),
_pRootCell	(NULL),
_nNbrCells	(0),
_nDepth		(0),
_rHeight	(5.0f)
...
void	QUADTREE::ComInit(CLASS_DESCRIPTOR * const	pCD)
{
	...
	static_cast<COMMUNICATOR_DESCRIPTOR *> (pCD)->AddReadChunk("QUADTREE", 1, CM_ReadChunk);
#endif

#ifndef _MASTER
	static_cast<COMMUNICATOR_DESCRIPTOR *> (pCD)->AddWriteChunk("QUADTREE", 1, CM_WriteChunk);
#endif
}
...
{
	return ( COMMUNICATOR_DESCRIPTOR::FindComDescriptor(MOTEUR_3D_MODULE::OTYPE_MOTEUR3D_QUADTREE) );
}
...
//-----------------------------------------------------------------------------
COMMUNICATOR_DESCRIPTOR *	QUADTREE::CreateGenericCommunicatorDescriptor()
{
	return( new COMMUNICATOR_DESCRIPTOR(QDT_COMMENT("QUADTREE"),
										MOTEUR_3D_MODULE::OTYPE_MOTEUR3D_QUADTREE,
										"2F002168-B1CD-39CA-A64E-434FF4A8B788",
										sizeof(QUADTREE),
										ComInit,
										QDT_COMMENT("Class for quadtrees"),
...
void	QUADTREE_CELL::Display(float rHeight, 
							   QDT_VECTOR <SEGMENT> & vSegments)
{
	VECTOR p1(GetXMin(), rHeight, GetYMin());
	VECTOR p2(GetXMax(), rHeight, GetYMin());
	VECTOR p3(GetXMin(), rHeight, GetYMax());
	VECTOR p4(GetXMax(), rHeight, GetYMax());
